...
function(module) {
	...
	Timespan.prototype.toString = function(max) {
		...
		if (count > 1) {
			label[count-1] = 'et'+label[count-1]; /* et = and*/
		}
		return label.join(', ');
	};
	...
	Timespan.prototype.toHTML = function(tag, max) {
		...
		if (--count) {
			label[count] = 'et '+label[count];/* et = and*/
		}
		return label.join(', ');
	};
	...
	formatList = function(ts) {
		...
		if (ts.days) {
			list.push(plurality(ts.days, 'j', 'j')); /*days*/
		}
		if (ts.hours) {
			list.push(plurality(ts.hours, 'h', 'h')); /*hours*/
		}
		if (ts.minutes) {
			list.push(plurality(ts.minutes, 'min', 'min')); /*minutes*/
		}
		if (ts.seconds) {
			list.push(plurality(ts.seconds, 's', 's')); /*seconds*/
		}
		if (ts.milliseconds) {
